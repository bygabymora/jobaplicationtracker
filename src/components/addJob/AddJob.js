...
import './addJob.css';
import { BsClipboardPlus, BsX } from 'react-icons/bs';
import FileUploader from '../fileUploader/FileUploader';
import Select from 'react-select';


const AddJob = ({jobList, setJobList}) => {
 ...
 const handleSelectChange = (e) => {
  ...
  return (
    ...
    py-1 mx-1.5 pl-2 pr-2.5 rounded hover:opacity-70 new__button' 
    type='button'
      ...
      <>
        ...
        <div className='w-9/12 max-w-md border rounded-lg shadow-md relative flex flex-col add__new__application'>
          <div className='flex flex-row justify-between p-5 border'>
            <h3 className='h3__title'>Add New Job Application  <BsClipboardPlus/> </h3>
            <button
            className='px-1 text-gray-400 float-right text-3xl leading-none 
            font-semibold block' onClick={() => setAddModal(false)}
            > 
            <BsX/>
            </button>
          </div>
           <form className='p-6'>
                  ...
                  <p className='error__message'>{errorMessages}</p>
                </form>
             <div className='flex flex-row justify-between p-5 border'>
